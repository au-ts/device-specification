...
Definition i2c_read_def:
  i2c_read (st: i2c_state) (nb: num) (offset: num) =
    ...
    in
      ...
      | 0x8 => INR (st, 0w : word32)
      | 0xc => INR (st, 0w : word32)
      | 0x10 => INR (st, (w2w st.regs.ctrl.enablehost << 0) || (w2w st.regs.ctrl.enabletarget << 1) || (w2w st.regs.ctrl.llpbk << 2) : word32)
      | 0x14 => INR (st, (w2w (i2c_get_status_fmtfull st) << 0) || (w2w (i2c_get_status_rxfull st) << 1) || (w2w (i2c_get_status_fmtempty st) << 2) || (w2w (i2c_get_status_hostidle st) << 3) || (w2w (i2c_get_status_targetidle st) << 4) || (w2w (i2c_get_status_rxempty st) << 5) || (w2w (i2c_get_status_txfull st) << 6) || (w2w (i2c_get_status_acqfull st) << 7) || (w2w (i2c_get_status_txempty st) << 8) || (w2w (i2c_get_status_acqempty st) << 9) : word32)
      | 0x18 => INR (i2c_rdata_read st, (w2w (i2c_get_rdata_rdata st) << 0) : word32)
      | 0x1c => INR (st, 0w : word32)
      | 0x20 => INR (st, (w2w st.regs.fifo_ctrl.rxilvl << 2) || (w2w st.regs.fifo_ctrl.fmtilvl << 5) : word32)
      | 0x24 => INR (st, (w2w (i2c_get_fifo_status_fmtlvl st) << 0) || (w2w (i2c_get_fifo_status_txlvl st) << 8) || (w2w (i2c_get_fifo_status_rxlvl st) << 16) || (w2w (i2c_get_fifo_status_acqlvl st) << 24) : word32)
      | 0x28 => INR (st, (w2w st.regs.ovrd.txovrden << 0) || (w2w st.regs.ovrd.sclval << 1) || (w2w st.regs.ovrd.sdaval << 2) : word32)
      | 0x2c => INR (st, (w2w (i2c_get_val_scl_rx st) << 0) || (w2w (i2c_get_val_sda_rx st) << 16) : word32)
      | 0x30 => INR (st, (w2w st.regs.timing0.thigh << 0) || (w2w st.regs.timing0.tlow << 16) : word32)
      | 0x34 => INR (st, (w2w st.regs.timing1.t_r << 0) || (w2w st.regs.timing1.t_f << 16) : word32)
      | 0x38 => INR (st, (w2w st.regs.timing2.tsu_sta << 0) || (w2w st.regs.timing2.thd_sta << 16) : word32)
      | 0x3c => INR (st, (w2w st.regs.timing3.tsu_dat << 0) || (w2w st.regs.timing3.thd_dat << 16) : word32)
      | 0x40 => INR (st, (w2w st.regs.timing4.tsu_sto << 0) || (w2w st.regs.timing4.t_buf << 16) : word32)
      | 0x44 => INR (st, (w2w st.regs.timeout_ctrl.val << 0) || (w2w st.regs.timeout_ctrl.en << 31) : word32)
      | 0x48 => INR (st, (w2w st.regs.target_id.address0 << 0) || (w2w st.regs.target_id.mask0 << 7) || (w2w st.regs.target_id.address1 << 14) || (w2w st.regs.target_id.mask1 << 21) : word32)
      | 0x4c => INR (i2c_acqdata_read st, (w2w (i2c_get_acqdata_abyte st) << 0) || (w2w (i2c_get_acqdata_signal st) << 8) : word32)
      | 0x50 => INR (st, 0w : word32)
      | 0x54 => INR (st, (w2w st.regs.host_timeout_ctrl.host_timeout_ctrl << 0) : word32)
      | _ => INL FFI_failed
End

Definition i2c_write_def:
  i2c_write (st: i2c_state) (nb: num) (offset: num) (value: word32) =
    let
      st = apply_fbits st
    in
      case offset of
        0x0 =>
          let
            st = st with regs := st.regs with intr_state := st.regs.intr_state with <|
            ...
            |>;
          in
            if nb >= 2 then INR st else INL FFI_failed
      | 0x4 =>
          let
            st = st with regs := st.regs with intr_enable := st.regs.intr_enable with <|
      ...
      | 0x8 =>
          let
            st = i2c_intr_test_written st;
          in
            if nb >= 2 then INR st else INL FFI_failed
      | 0xc =>
          let
            st = i2c_alert_test_written st;
          in
            if nb >= 1 then INR st else INL FFI_failed
      | 0x10 =>
          let
            st = st with regs := st.regs with ctrl := st.regs.ctrl with <|
          ...
          in
            if nb >= 1 then INR st else INL FFI_failed
      | 0x14 => if nb >= 2 then INR st else INL FFI_failed
      | 0x18 => if nb >= 1 then INR st else INL FFI_failed
      | 0x1c =>
          let
            st = st with regs := st.regs with fdata := st.regs.fdata with <|
              fbyte := w2w ((7 -- 0) value);
              start := w2w ((8 -- 8) value);
              stop := w2w ((9 -- 9) value);
            ...
            |>;
            st = i2c_fdata_written st;
          in
            if nb >= 2 then INR st else INL FFI_failed
      | 0x20 =>
          let
            st = st with regs := st.regs with fifo_ctrl := st.regs.fifo_ctrl with <|
              rxrst := w2w ((0 -- 0) value);
              ...
              txrst := w2w ((8 -- 8) value);
            |>;
            st = i2c_fifo_ctrl_written st;
          in
            if nb >= 2 then INR st else INL FFI_failed
      | 0x24 => if nb >= 4 then INR st else INL FFI_failed
      | 0x28 =>
          let
            st = st with regs := st.regs with ovrd := st.regs.ovrd with <|
          ...
          in
            if nb >= 1 then INR st else INL FFI_failed
      | 0x2c => if nb >= 4 then INR st else INL FFI_failed
      | 0x30 =>
          let
            st = st with regs := st.regs with timing0 := st.regs.timing0 with <|
              thigh := w2w ((15 -- 0) value);
              tlow := w2w ((31 -- 16) value);
            |>;
          in
            if nb >= 4 then INR st else INL FFI_failed
      | 0x34 =>
          let
            st = st with regs := st.regs with timing1 := st.regs.timing1 with <|
              t_r := w2w ((15 -- 0) value);
              t_f := w2w ((31 -- 16) value);
            |>;
          in
      ...
      | 0x38 =>
          let
            st = st with regs := st.regs with timing2 := st.regs.timing2 with <|
      ...
      | 0x3c =>
          let
            st = st with regs := st.regs with timing3 := st.regs.timing3 with <|
      ...
      | 0x40 =>
          let
            st = st with regs := st.regs with timing4 := st.regs.timing4 with <|
      ...
      | 0x44 =>
          let
            st = st with regs := st.regs with timeout_ctrl := st.regs.timeout_ctrl with <|
              val := w2w ((30 -- 0) value);
              en := w2w ((31 -- 31) value);
            |>;
          in
      ...
      | 0x48 =>
          let
            st = st with regs := st.regs with target_id := st.regs.target_id with <|
          ...
          in
            if nb >= 4 then INR st else INL FFI_failed
      | 0x4c => if nb >= 2 then INR st else INL FFI_failed
      | 0x50 =>
          let
            st = st with regs := st.regs with txdata := st.regs.txdata with <|
              txdata := w2w ((7 -- 0) value);
            |>;
            st = i2c_txdata_written st;
          in
            if nb >= 1 then INR st else INL FFI_failed
      | 0x54 =>
          let
            st = st with regs := st.regs with host_timeout_ctrl := st.regs.host_timeout_ctrl with <|
